...
async function fetchWithFallback(endpoint, cleanNumericId, OB_KEY, OB_SECRET, timeoutMs) {
  ...
  for (const gateway of OB_GATEWAYS) {
    ...
    if (is4005Expired(res.data)) {
      ...
      if (endpoint === '1688') {
        console.log('[1688-item-detail] Switching to 1688global endpoint...')
        return await fetchWithFallback('1688global', cleanNumericId, OB_KEY, OB_SECRET, timeoutMs)
      }
      console.error('[1688-item-detail] 1688global also returned 4005. Giving up.')
...
export default async function handler(req, res) {
  ...
  const result = await fetchWithFallback('1688', cleanNumericId, OB_KEY, OB_SECRET, 18000)
