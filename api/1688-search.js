  ...
  for (const gateway of OB_GATEWAYS) {
    ...
    try {
      ...
      try { data = await r.json() } catch (je) {
        ...
        console.warn('[1688-search] JSON parse error from ' + gateway + ' status=' + r.status + ' body:', text.slice(0, 200))
        lastErr = new Error('JSON parse failed (HTTP ' + r.status + ')')
        continue
      }
      if (!data) { lastErr = new Error('Empty response from ' + gateway); continue }
      console.log('[1688-search] OK from ' + gateway + ' HTTP ' + r.status + ' keys:', Object.keys(data).slice(0, 8))
      ...
      console.error('[1688-search] Fetch error from ' + gateway + ': ' + reason)
      if (err.stack) console.error('[1688-search] Stack:', err.stack.split('\n').slice(0, 3).join(' | '))
      lastErr = err
    }
  }
  const msg = lastErr ? lastErr.message : 'All OneBound gateways failed'
  console.error('[1688-search] ALL gateways failed. Last error:', msg)
...
export default async function handler(req, res) {
  ...
  const path = '/1688/item_search/?key=' + OB_KEY + '&secret=' + OB_SECRET + '&q=' + encodeURIComponent(queryZh) + '&page=' + page + '&result_type=json'
