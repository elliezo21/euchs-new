    ...
    >
      <div class="bg-white rounded-3xl max-w-2xl w-full p-6 sm:p-7 shadow-2xl space-y-6 max-h-[92vh] overflow-y-auto text-xs text-gray-700">
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-200">
          ...
          <button @click="closeOrderModal" class="text-gray-400 hover:text-gray-900 p-1.5 rounded-xl hover:bg-gray-100 transition cursor-pointer">
            <X class="w-5 h-5" />
          </button>
        </div>
        ...
        <div class="flex items-center justify-end gap-2 pt-2 border-t border-gray-100">
          ...
          >
            <Loader2 v-if="isSubmittingOrder" class="w-4 h-4 animate-spin" />
            <Send v-else class="w-4 h-4" />
            <span>{{ isSubmittingOrder ? '발주서 접수 중...' : '수입 발주서 최종 제출 (견적 요청)' }}</span>
          </button>
        </div>
      </div>
    </div>
